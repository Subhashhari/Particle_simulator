...
# Generate JNI headers and compile all Java classes in one step
$(OUT_DIR)/Pack/Particle/Particle.class: $(SRC_JAVA)/Pack/Particle/Particle.java $(SRC_JAVA)/Pack/FieldPoint/FieldPoint.java $(SRC_JAVA)/Pack/Main.java $(SRC_JAVA)/Pack/CanvasTest.java 
	mkdir -p $(HEADERS_DIR) $(OUT_DIR)
	javac -h $(HEADERS_DIR) -d $(OUT_DIR) -sourcepath $(SRC_JAVA) $(SRC_JAVA)/Pack/Main.java
